...
class WeaponSkinLevelApiRepository @Inject constructor(
    ...
    override suspend fun getWeaponSkinLevel(
        ...
        return gameDatabase.weaponSkinLevelDao.getWeaponSkinLevel(uuid).distinctUntilChanged()
            .transform { entity ->
                if (entity == null || entity.version != version) {
                    fetchWeaponSkinLevel(uuid, version)
                } else {
                    emit(entity)
                }
            }.map { it.toType() }
    ...
    override suspend fun fetchWeaponSkinLevel(uuid: String, version: String) {
