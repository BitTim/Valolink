...
class BuddyLevelApiRepository @Inject constructor(
    ...
    override suspend fun getBuddyLevel(uuid: String, providedVersion: String?): Flow<BuddyLevel> {
        val version = providedVersion ?: versionRepository.getApiVersion()?.version ?: ""

        return gameDatabase.buddyLevelDao.getBuddyLevel(uuid).distinctUntilChanged()
            .transform { entity ->
                if (entity == null || entity.version != version) {
                    fetchBuddyLevel(uuid, version)
                } else {
                    emit(entity)
                }
            }.map { it.toType() }
    ...
    override suspend fun fetchBuddyLevel(uuid: String, version: String) {
