...
class PlayerCardApiRepository @Inject constructor(
    ...
    override suspend fun getPlayerCard(uuid: String, providedVersion: String?): Flow<PlayerCard> {
        val version = providedVersion ?: versionRepository.getApiVersion()?.version ?: ""

        return gameDatabase.playerCardDao.getPlayerCard(uuid).distinctUntilChanged()
            .transform { entity ->
                if (entity == null || entity.version != version) {
                    fetchPlayerCard(uuid, version)
                } else {
                    emit(entity)
                }
            }.map { it.toType() }
    ...
    override suspend fun fetchPlayerCard(uuid: String, version: String) {
