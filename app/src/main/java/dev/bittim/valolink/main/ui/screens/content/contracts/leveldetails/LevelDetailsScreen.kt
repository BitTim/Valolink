...
fun LevelDetailsScreen(
    ...
    if (state.level != null && state.level.reward.relation != null && state.contract != null) {
        ...
        val isLocked = !userContract.levels.any { it.level == state.level.uuid }
        ...
        if (isRewardUnlockAlertShown && !userContract.levels.any { it.level == state.level.uuid }) {
        ...
        if (isRewardResetAlertShown && userContract.levels.any { it.level == state.level.uuid }) {
            val stagedLevels = Level.reverseTraverse(
                contractLevels,
                userContract.levels.lastOrNull()?.level,
                state.level.uuid,
