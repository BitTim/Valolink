...
class SeasonSyncWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        ...
        val localVersions = gameDatabase.seasonDao.getAll().firstOrNull()?.map { it.version }
        ...
        if (localVersions == null || localVersions.any { it != remoteVersion }) {
            if (uuid.isNullOrEmpty()) seasonRepository.fetchAll(remoteVersion)
            else seasonRepository.fetch(
                uuid,
                remoteVersion
            )
        }
