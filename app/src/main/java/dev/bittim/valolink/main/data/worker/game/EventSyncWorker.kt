...
class EventSyncWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        ...
        val localVersions = gameDatabase.eventDao.getAll().firstOrNull()?.map { it.version }
        ...
        if (localVersions == null || localVersions.any { it != remoteVersion }) {
            if (uuid.isNullOrEmpty()) eventRepository.fetchAll(remoteVersion)
            else eventRepository.fetch(
                uuid,
                remoteVersion
            )
        }
