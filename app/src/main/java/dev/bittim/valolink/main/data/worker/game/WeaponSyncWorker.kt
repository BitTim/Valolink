...
class WeaponSyncWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        ...
        val localVersions = gameDatabase.weaponDao.getAll().firstOrNull()?.map { it.weapon.version }
        ...
        if (localVersions == null || localVersions.any { it != remoteVersion }) {
            if (uuid.isNullOrEmpty()) weaponRepository.fetchAll(remoteVersion)
            else weaponRepository.fetch(
                uuid,
                remoteVersion
            )
        }
