...
class SpraySyncWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        ...
        val localVersions = gameDatabase.sprayDao.getAll().firstOrNull()?.map { it.version }
        ...
        if (localVersions == null || localVersions.any { it != remoteVersion }) {
            if (uuid.isNullOrEmpty()) sprayRepository.fetchAll(remoteVersion)
            else sprayRepository.fetch(
                uuid,
                remoteVersion
            )
        }
