...
class CurrencySyncWorker @AssistedInject constructor(
    ...
    override suspend fun doWork(): Result {
        ...
        val localVersions = gameDatabase.currencyDao.getAll().firstOrNull()?.map { it.version }
        ...
        if (localVersions == null || localVersions.any { it != remoteVersion }) {
            if (uuid.isNullOrEmpty()) currencyRepository.fetchAll(remoteVersion)
            else currencyRepository.fetch(
                uuid,
                remoteVersion
            )
        }
