...
class WeaponApiRepository @Inject constructor(
	...
	override suspend fun getByUuid(uuid: String): Flow<Weapon?> {
		return try {
			...
			val local =
				gameDatabase.weaponDao.getByUuid(uuid).distinctUntilChanged().map { it?.toType() }
	...
	override suspend fun getSkinByLevelUuid(
		...
		return try {
			// Get from local database
			val local = gameDatabase.weaponDao.getSkinByLevelUuid(levelUuid).distinctUntilChanged()
				.map { it?.toType() }
	...
	override suspend fun getAll(): Flow<List<Weapon>> {
		return try {
			// Get from local database
			val local = gameDatabase.weaponDao.getAll().distinctUntilChanged().map { entities ->
