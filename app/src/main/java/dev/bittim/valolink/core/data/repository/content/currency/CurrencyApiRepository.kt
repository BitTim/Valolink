package dev.bittim.valolink.core.data.repository.content.currency
...
class CurrencyApiRepository @Inject constructor(
	...
	override suspend fun getByUuid(
		...
		return try {
			...
			val local =
				contentDatabase.currencyDao.getByUuid(uuid).distinctUntilChanged()
					.map { it?.toType() }
	...
	override suspend fun getAll(
		...
		return try {
			// Get from local database
			val local =
				contentDatabase.currencyDao.getAll().distinctUntilChanged().map { entities ->
	...
	override suspend fun fetch(
		...
		val response = contentApi.getCurrency(uuid)
		if (response.isSuccessful) {
			contentDatabase.currencyDao.upsert(response.body()!!.data!!.toEntity(version))
		}
	}
	...
	override suspend fun fetchAll(
		...
		val response = contentApi.getAllCurrencies()
		if (response.isSuccessful) {
			contentDatabase.currencyDao.upsert(
	...
	override fun queueWorker(
		...
		val workRequest = OneTimeWorkRequestBuilder<ContentSyncWorker>().setInputData(
			workDataOf(
				ContentSyncWorker.KEY_TYPE to Currency::class.simpleName,
				ContentSyncWorker.KEY_UUID to uuid,
			)
		).setConstraints(Constraints(NetworkType.CONNECTED)).build()
		workManager.enqueueUniqueWork(
			Currency::class.simpleName + ContentSyncWorker.WORK_BASE_NAME + if (!uuid.isNullOrEmpty()) "_$uuid" else "",
